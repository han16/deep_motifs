...
# Scatter plot
ggplot(highlight_terms_wide, aes(x = -log10(`pvalue_Deep-MOTIFs genes`), y = -log10(`pvalue_SFARI genes`))) +
  geom_point(aes(), alpha = 0.6, size = 2) +
  scale_color_manual(values = c("grey70", "red")) +
  geom_abline(slope = 1, intercept = 0, linetype = "solid", color="red") +
  labs(
    title = "Comparison of -log10(p-values)",
    x = "-log10(p) Deep-MOTIFs",
    y = "-log10(p) SFARI",
    color = "Highlighted"
  )+
   theme(plot.title = element_text(hjust = 0.5, size=12))  #center the title 
