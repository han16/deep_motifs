...
#write.csv(wide_df, file="C:\\Users\\hans\\OneDrive - Marquette University\\AI_for_Autism\\supp\\KEGG_pathway.csv")
...
wide_df %>% filter(`FoldEnrichment.Deep-MOTIFs`>`FoldEnrichment.SFARI`)
wide_df %>% filter(`FoldEnrichment.Deep-MOTIFs`<`FoldEnrichment.SFARI`)
wide_df %>% filter(is.na(`FoldEnrichment.Deep-MOTIFs`)==T & is.na(`FoldEnrichment.SFARI`)==F)
wide_df %>% filter(is.na(`FoldEnrichment.Deep-MOTIFs`)==F & is.na(`FoldEnrichment.SFARI`)==T)
```
